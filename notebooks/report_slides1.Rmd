...
- Also, since PTSD is only moderately heritable (h^2≈30%-70% depending on gender), a genetic score by itself is inherently limited.
...
## Constructing PRS
- PRS computation was done using <span style="color: blue;"> pruning and thresholding </span> (P+T) approach as implemented on PRSice.
- GWAS summary statistic from PGC-PTSD (<span style="color: blue;">freeze-2</span>) study was used.
...
- <span style="color: blue;">PTSD status</span> is the phenotype of interest, but the diagnosis method varies among the four cohort.
...
- Training/discovery cohort: SysBio (n=145)
- Testing/replication cohort: 
  FC (n=199), GTP (n=5,436), AURORA (n=507)
- Response variable: PTSD diagnosis
- Predictor variables: 
	- PRS (polygenic risk score discretized to 4 levels)
	- EA4  (educational attainment discretized to 4 levels)
	- CA4 (childhood trauma discretized to 4 levels)
